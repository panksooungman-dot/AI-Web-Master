...
> 최종 분석: 2026-08-10 (Claude Code, apps/**·packages/** 변경 자동 반영 — 커밋 `7b83ce6` 기준)
> 커밋 `edba62a` 기준)
...
| AI 홈페이지 생성기(Website Builder v2) | 93% | CLI+대시보드 완결. **Design Automation 9단계 개발 프로세스 중 04 DB/05 API/06 Backend/08 Test 4개 Phase를 문서 산출물에서 실제 실행 가능한 코드(Route Handler·SQL DDL/RLS·서비스 함수·통합 테스트)로 완성**하고, **Chain A(Review 기반, Website Build) ↔ Chain B(Plan 기반, Database/API/Backend/Test Code) 연결**(`lib/design/website-fullstack-adapter.ts`)까지 구현. **07 CRUD Frontend Generator**(`lib/design/crud-frontend{,-generator}.ts`, 신규)로 Backend Design+Database Design을 조합해 실제 백엔드 API를 호출하는 목록/등록/수정 화면까지 완성했고, **9-Stage Orchestrator(Chain B)**(`POST /api/design/orchestrate`, 신규)가 Design Plan부터 CRUD Frontend까지 10개 산출물 전체를 요구사항 한 번 입력으로 순서대로 생성하도록 체이닝했다. **범위 사전 검증(scope-check)**(`lib/design/scope-check.ts`, 신규)이 Database/API Design 생성 직후 남은 5개 배치 기반 단계의 예상 AI 호출량을 실제 `BATCH_SIZE` 상수로 정확히 추정해 임계값(40회)을 넘으면 `completed:false`로 멈추고 Multi-Step 진행을 안내하는(또는 `force:true`로 강행) 방식으로 전환해, 규모와 무관하게 항상 끝까지 자동 실행되던 이전 동작을 개선했다. **Chain A Orchestrator**(`POST /api/design/design-orchestrate`, 신규, `lib/design/design-chain-orchestrator.ts`)도 추가해 Chain B의 9-Stage Orchestrator와 대응하는 Requirements→Storyboard→Wireframe→Prototype→Claude Design 5단계를 한 번의 호출로 생성하고, 그 위에 Review를 항상 `in_review` 상태로 시작해 둔다(규모와 무관하게 항상 Review에서 멈춰 승인·Figma Export·Design Sync·Website Build는 여전히 사람의 판단 하에 별도로 진행). `outputFileTracingIncludes`에서 CLI 호출 라우트가 처음부터 누락돼 있던 프로덕션 버그도 함께 수정. 실 subprocess E2E(vitest 서브프로세스로 생성 코드를 직접 실행)로 auth-guard try-블록 배치 버그·한글 리소스명 슬러그 충돌 버그를 재현·수정·재검증 완료 |
...
| 테스트 인프라 | 100% | `apps/cnbiz-web` 실 계정 E2E 검증 중 발견된 버그에 대한 신규/보강 테스트 포함해 전부 통과. Orchestrator는 결정론적 폴백 기준 통합 테스트로 체이닝 정확성 및 범위 사전 검증(scope-check) 경로까지 검증(실 AI 크레딧 미사용) |
...
- **Design Automation — 07 CRUD Frontend Generator + Chain B 9-Stage Orchestrator**(`lib/design/crud-frontend{,-generator}.ts`, `lib/design/orchestrator.ts`, 신규) — API Code(`apiCodeId`)로부터 그 체인을 거슬러 Backend Design(serviceFunction 이름)·Database Design(컬럼 정보)을 함께 조회해 리소스마다 실제 백엔드 API를 호출하는 목록/등록/수정 화면을 생성한다(AI 미사용, 순수 기계적 변환). `POST/GET /api/design/crud-frontend`·`GET /api/design/crud-frontend/[id]` 신규. `runDesignOrchestration()`(`lib/design/orchestrator.ts`)이 Design Plan→Database→API→Backend(Design+Code)→API Code→Database Code→Test Plan→Test Code→CRUD Frontend까지 10개 산출물을 하나의 요구사항 입력으로 순서대로 생성하도록 체이닝하며, `POST /api/design/orchestrate`가 각 단계별로 개별 호출했을 때와 동일한 Audit Log 액션·Metrics 카운터를 그대로 기록하고 `design.orchestrate.run`(신규 액션)·`orchestrationRunCount`(신규 카운터)로 전체 체인 실행 자체도 별도 기록한다. **범위 사전 검증(scope-check)**(`lib/design/scope-check.ts`, 신규)이 Database/API Design 생성 직후 남은 5개 배치 기반 단계의 예상 AI 호출량을 정확히 추정해 임계값(40회) 초과 시 `completed:false`로 멈추고 Multi-Step 안내(또는 `force:true` 강행)를 반환한다. `next.config.ts`의 `outputFileTracingIncludes`에서 이 신규 라우트를 포함해 여러 CLI 호출 라우트가 처음부터 빠져 있던 프로덕션 버그(Vercel에서 `require()` 단계부터 조용히 실패)도 함께 수정
- **Design Automation — Chain A Orchestrator**(`lib/design/design-chain-orchestrator.ts`, `POST /api/design/design-orchestrate`, 신규) — Chain B의 9-Stage Orchestrator와 대응하는 Chain A(Review 기반) 자동화. Requirements→Storyboard→Wireframe→Prototype→Claude Design 5단계를 한 번의 요청으로 생성하고 그 위에 Review를 `in_review` 상태로 시작해 둔다(규모와 무관하게 항상 Review에서 멈춤 — 승인·Figma Export·Design Sync는 사람이 별도로 진행). 각 단계는 개별 라우트 호출과 동일한 Audit Log 액션·Metrics 카운터를 기록하며, `design.chain-orchestrate.run`(신규 액션)·`designChainOrchestrationRunCount`(신규 카운터)로 전체 체인 실행 자체도 별도 집계한다
...
- **Chain A Orchestrator — `POST /api/design/design-orchestrate` 신규 구현**(2026-08-11) — `lib/design/design-chain-orchestrator.ts`(신규, `runDesignChainOrchestration()`)가 2026-08-11에 만든 Chain B 9-Stage Orchestrator(`/api/design/orchestrate`)의 대응물로, Requirements→Storyboard→Wireframe→Prototype→Claude Design 5단계를 한 번의 호출로 순서대로 생성한다. Chain B의 규모 검사와 달리 Chain A는 규모와 무관하게 Claude Design 다음 Review(`createReview()`, `in_review` 상태) 생성까지만 하고 승인은 대신하지 않는다(Review는 사람이 판단하는 지점이라는 기존 설계를 그대로 존중, Figma Export·Design Sync는 승인 이후 별도 진행). `app/api/design/design-orchestrate/route.ts`(신규)가 각 단계별로 기존 개별 라우트 호출과 동일한 Audit Log 액션·Metrics 카운터를 기록하고, 신규 `design.chain-orchestrate.run` Audit Action + `designChainOrchestrationRunCount` Metrics 카운터로 "5단계를 한 번에 실행했다"는 사실을 별도 기록한다. `next.config.ts`의 `outputFileTracingIncludes`에 신규 라우트 추가, 신규 테스트 4개(`tests/design/design-chain-orchestrator.test.ts`)로 FK 체인·Review 항상 `in_review`·actor 전달·같은 입력 재실행 시 독립적 버전 관리 검증.
- **9-Stage Orchestrator — 범위 사전 검증(scope-check) 추가, 규모 무관 전체 실행에서 Multi-Step Workflow로 전환**(2026-08-11) — `lib/design/scope-check.ts`(신규)의 `estimateScope()`가 Database Design·API Design 생성 직후(아직 배치 기반 단계는 시작 전) 남은 5개 단계(Backend Design/Code·Database Code·Test Plan/Code)의 예상 AI 호출(배치) 총합을 각 생성기의 실제 `BATCH_SIZE` 상수(신규 `export`로 재사용)와 결정론적 관계로 정확히 계산한다. `AUTO_RUN_BATCH_LIMIT`(40)을 넘으면 `lib/design/orchestrator.ts`가 API Design 단계에서 멈추고 `completed:false` + 안내 메시지(`apiDesignId`부터 개별 진행하거나 `force:true`로 강행)를 반환하도록 확장했고, `app/api/design/orchestrate/route.ts`는 이 경우 실제 생성된 Design Plan·Database Design·API Design 3개 단계의 Audit/Metrics만 기록하며 신규 Audit Action(`design.orchestrate.scope_stop`)·Metrics 카운터(`orchestrationScopeStopCount`)로 중단 사실을 별도 기록한다. 신규 테스트 9개(`tests/design/scope-check.test.ts` 4개 + `orchestrator.test.ts` 5개 보강). 임계값은 최초 15로 어림잡았다가, 정상 회귀 테스트 실행 중 Design Plan의 결정론적 기본 폴백만으로도 이미 18회가 나와 가장 작은 기본 케이스조차 즉시 멈추는 결함을 발견해 40으로 재보정했다.
