...
`apps/cnbiz-web`(실제 배포 대상) 기준, 아래 항목 전부 통과 확인(의뢰 접수 시스템 추가 반영):

| 항목 | 결과 |
|---|---|
| `npm test` | ✅ 51/51 파일, 409/409 테스트 통과 |
| `npm run lint` | ✅ 0 errors, 0 warnings |
| `npm run build` | ✅ 98개 라우트 정상 생성 (Turbopack 경고 1건, 아래 참고) |
| TypeScript (`tsc --noEmit`) | ✅ 0 errors |
| API 오류 | ✅ 84개 API 라우트 전부 유효한 핸들러 확인. **치명적 버그 1건 발견·수정**(아래) |
...
**이후 추가된 기능(같은 날, 아래 참고):** 의뢰 접수 시스템(`/request` → `lib/requests/*` → `/developer/requests`) 신규 구현. 신규 API 3개(`POST /api/requests/submit`(공개), `GET /api/requests`·`GET/PATCH /api/requests/[id]`(관리자 전용))도 이번 릴리스 체크(lint/build/tsc/test) 대상에 포함되어 전부 통과.
...
- 페이지: `/`(Home), `/about`, `/services`, `/portfolio`(placeholder), `/contact`, `/request`(신규, 제작 의뢰 접수), `/login`, `/signup`(폼만, 백엔드 없음)
- 문의 폼 → API(`/api/contact`) → 로컬 저장 + Resend 이메일 발송, honeypot·rate limit 스팸 방지
- **의뢰 접수 시스템(신규, 2026-07-18)** — `/request` 페이지(회사명·담당자명·이메일·연락처·업종·홈페이지 종류·필요한 기능(체크박스)·참고 사이트·예산·요청사항 10개 입력) → `POST /api/requests/submit`(공개, honeypot·rate limit 적용) → 서버 검증 → `lib/requests/registry.ts`(CollectionStore, `getDefaultStore()`로 로컬/Supabase 자동 전환) 저장 → 성공/실패 메시지. Playwright 실 브라우저로 제출·검증 오류·honeypot·모바일(390px) 레이아웃까지 확인
- SEO: sitemap.xml, robots.txt, Organization JSON-LD, Google Analytics 4

### Development OS 대시보드 (`apps/cnbiz-web/app/developer/**`, 총 18개 모듈)
Terminal, Workspace, GitHub, AI Workspace(+Tasks), Website Builder, Workflow Center, Marketplace(+Installed/Updates/상세), Settings, Logs, Health, Audit Log, Metrics, Backup, Error Report, Design Automation(9개 하위 페이지), **의뢰 관리(신규)** — 전부 실제 API와 연결되어 동작.
...
- 실제 존재하는 관리 모듈: Terminal, Workspace, GitHub, AI Workspace, Website Builder, Workflow Center, Marketplace, Settings, Logs, Health, Audit Log, Metrics, Backup, Error Report, Design Automation 9종, **의뢰 관리(신규, `/developer/requests`)**.
- **의뢰 관리(신규)**: `/developer/requests`(목록, 상태별 필터: 신규/검토중/진행중/반려/완료) + `/developer/requests/[id]`(상세 — 연락처·예산·필요한 기능·요청사항 전체 표시, 상태 변경 버튼). `/request` 페이지에서 접수된 건이 실시간으로 나타남(Playwright로 접수→목록→상세→상태 변경까지 전 구간 실 브라우저 확인).
...
- 고객이 접하는 것은 CNBIZ Website v2 공개 페이지(Home/About/Services/Portfolio/Contact/**Request(신규)**)와 문의 폼·**제작 의뢰 폼(신규)**.
- 문의 접수 → 로컬 JSON 저장 + 이메일 알림(Resend)까지 구현 완료.
- **제작 의뢰 접수(신규, 2026-07-18)** — `/request`에서 회사명·담당자명·연락처·업종·홈페이지 종류·필요한 기능·참고 사이트·예산·요청사항을 남기면 서버 검증 후 저장, 관리자가 `/developer/requests`에서 확인·상태 관리. 이메일 알림은 이번 범위에 포함하지 않음(요청사항에 명시되지 않아 범위 밖으로 판단).
- 고객 전용 로그인/마이페이지/프로젝트 진행 조회 같은 "고객 포털" 개념은 여전히 존재하지 않음 — `/login`은 관리자(Development OS) 로그인이며 고객용이 아님. 의뢰 상태(신규/검토중/진행중/반려/완료)는 관리자만 확인 가능하고 고객이 직접 조회하는 기능은 없음.
...
- 로컬 fs 기반 registry(`lib/data/*.json`, 런타임 생성): projects, workspaces, prompts, workflows, sessions, users, audit-log, contact-submissions, **project-requests(신규)** 등.
- **(신규)** `lib/requests/registry.ts` — `project-requests` collection, `getDefaultStore()` 재사용(신규 스토리지 구현 없음, 기존 CollectionStore 추상화 그대로 사용 → Supabase 설정 시 자동으로 Supabase에 저장됨).

### API 라우트
- `apps/cnbiz-web/app/api/**`에 **84개** route.ts 파일 존재 — agents, ai, audit, auth(login/logout/me — signup 없음), backup, contact, design(10개 하위 리소스), dev-inspector, devserver, errors, health, logs, marketplace, metrics, projects, prompts, providers, **requests(신규, 3개)**, sessions, terminal, websites, workflows, workspaces 전 영역 커버.
- 루트 `app/`에는 API 라우트가 하나도 없음(레거시 정적 사이트라 불필요).
- **(2026-07-18 수정)** `/api/contact`가 RBAC 게이트에 걸려 무인증 방문자에게 401을 반환하던 치명적 버그를 발견·수정(0번 섹션 참고). 이 API 목록 중 `/api/contact`·`/api/requests/submit`만 공개 사이트가 직접 호출하는 라우트이고 나머지는 전부 `/developer` 대시보드 전용.
- **(신규)** `POST /api/requests/submit`(공개, 무인증) / `GET /api/requests`·`GET,PATCH /api/requests/[id]`(관리자 전용, RBAC 기본 게이트 — `UNGATED_API_PREFIXES`에 의도적으로 포함하지 않음). 같은 prefix 안에서 공개·관리자 라우트를 분리한 첫 사례라 `/api/requests/submit`만 정확히 예외 처리(`/api/requests` 자체는 prefix 매칭으로 인해 함께 풀리지 않도록 별도 서브패스로 설계, `lib/auth/rbac.ts` 주석 참고).
...
| 영역 | 진행률 | 근거 |
|---|---|---|
| CNBIZ Website v2 공개 페이지 | 85% | Portfolio·Contact 정보 TODO, 반응형/접근성 전수 QA 미완료. 의뢰 접수(`/request`) 신규 완료로 상승 |
| Development OS 대시보드 | 92% | 18개 모듈 전부 실동작(의뢰 관리 신규 포함), 관리자 UI 내 역할관리만 없음 |
...
| 고객(의뢰자) 시스템 | 60% | 제작 의뢰 접수·관리자 확인·상태 관리까지 완료. 고객이 직접 자기 의뢰 상태를 조회하는 포털은 없음 |
| 테스트 인프라 | 100% | 루트 16/16(126 tests) + `apps/cnbiz-web` 51/51(409 tests) 전부 통과, 이관 완료 |
| **전체 종합** | **약 86%** | 기능 구현 폭넓게 완료, 테스트 인프라 정상화, 릴리스 체크리스트(lint/build/tsc/API/env) 전항목 통과, 의뢰 접수 시스템 신규 완료 |

---

## 11. 최근 완료 작업

- **의뢰 접수 시스템 구현 (2026-07-18)** — 고객이 홈페이지 제작을 신청하는 `/request` 페이지(회사명·담당자명·이메일·연락처·업종·홈페이지 종류·필요한 기능·참고 사이트·예산·요청사항) → 서버 검증 → `lib/requests/registry.ts`(CollectionStore, Supabase/fs 자동 전환) 저장 → `/developer/requests`(목록+상태 필터) · `/developer/requests/[id]`(상세+상태 변경) 관리자 페이지 신규 구현. 공유 `Select` 컴포넌트를 `packages/ui`에 신규 추가. Header/Footer/DeveloperNav에 링크 연결. 신규 API 3개(`/api/requests/submit` 공개, `/api/requests`·`/api/requests/[id]` 관리자 전용 — `lib/auth/rbac.ts`에 공개/관리자 라우트를 같은 prefix 아래 분리하는 패턴 신규 적용). 테스트 3개 파일(validate/registry/spam) 신규 작성, `rbac.test.ts`·`proxy.test.ts`에 회귀 케이스 추가. Playwright 실 브라우저로 제출→접수 확인→관리자 목록→상세→상태 변경 전 구간 검증, 모바일(390px) 레이아웃 확인. `npm test`(51/51, 409 tests)·lint·build·tsc 전부 통과
...
1. **의뢰 접수 이메일 알림** — 현재 `/request` 제출 시 관리자에게 이메일 알림이 가지 않음(이번 요청 범위에 명시되지 않아 미포함). Contact의 `lib/contact/notify.ts`(Resend) 패턴을 그대로 재사용 가능.
2. **의뢰 → 실제 프로젝트 전환 UX** — 의뢰(Request)는 기존 Project Manager(워크스페이스/Git 기반, `/projects`)와 의도적으로 분리된 별개 엔티티로 설계됨(사용자 확인). 관리자가 실제 개발에 착수하기로 하면 현재는 `/projects`에서 수동으로 새 프로젝트를 생성해야 하며, 두 화면을 잇는 "프로젝트로 전환" 버튼 등은 아직 없음.
...
6. **`lib/terminal/server.ts` Turbopack 빌드 경고 근본 원인 조사** (0번 섹션 참고 — 빌드는 성공하나 원인 미해결로 남겨둠).
7. **WBS.md 최신화** — 현재 WBS는 2026-07-01/05 기준으로 동결되어 있어 이후의 대규모 구조 변경·Design Automation 9 Phase·의뢰 접수 시스템·이번 출시 준비 점검을 전혀 반영하지 못함.
