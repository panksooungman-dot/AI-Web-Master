...
import { getPrototypeJob } from "@/lib/design/prototypeJob";
import { runPrototypeJob } from "@/lib/design/prototypeJobWorker";

interface RouteParams {
  params: Promise<{ id: string }>;
}

// app/api/design/prototype/route.ts와 동일한 근거 — 이 라우트가 실제 생성을 수행하는 자리를
// 넘겨받았으므로 동일하게 300초로 맞춘다.
export const maxDuration = 300;

/**
 * app/api/design/requirements/jobs/[id]/run/route.ts와 동일한 패턴 — runPrototypeJob()은
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const job = await getPrototypeJob(id);
  if (!job) {
  ...
  const updated = await runPrototypeJob(id);
