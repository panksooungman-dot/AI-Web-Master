...
import { createEstimate, listEstimates } from "@/lib/estimates/registry";
...
export async function GET() {
  return NextResponse.json({ estimates: await listEstimates() });
}

/**
 * PROJECT_STATUS.md가 명시한 확장 지점 — AI Analysis Engine(lib/ai-analysis)의 AIAnalysisResult를
 * 입력으로 사용하는 "별도 서비스"로 구현했다(AiJobType 추가 방식 대신). Customer Inquiry
 * Pipeline(processJob()·AiJobType·AiJobStatus 등)은 전혀 건드리지 않는다 — 이 라우트는
 * Inquiry.analysis(이미 자동 생성되어 있음)를 읽기만 한다.
 */
export async function POST(request: Request) {
  ...
  const input: EstimateInput = {
    ...
    budget: websiteOrder?.budget,
  };

  const { result, simulated, provider, model } = await generateEstimate(input);
  ...
  await recordAuditEvent({
    ...
    actor,
    success: true,
    detail: `견적서 생성: "${input.companyName}" (${result.priceRangeMin.toLocaleString()}~${result.priceRangeMax.toLocaleString()}원)${simulated ? " (simulated)" : ""}`,
    metadata: { inquiryId: inquiry.id, websiteOrderId: inquiry.websiteOrderId, estimateId: record.id },
  });
  await incrementMetric("estimateGenerationCount");
