...
/** 최신순(newest first). */
export async function listFigmaRecords(store: CollectionStore = getDefaultStore()): Promise<FigmaRecord[]> {
  const records = await store.list<FigmaRecord>(COLLECTION);
  return [...records].reverse();
}

export async function getFigmaRecord(
  id: string,
  store: CollectionStore = getDefaultStore()
): Promise<FigmaRecord | null> {
  const records = await store.list<FigmaRecord>(COLLECTION);
  return records.find((record) => record.id === id) ?? null;
}

/** 특정 Review에 연결된 Figma 파일만(최신순). */
export async function listFigmaRecordsForReview(
  reviewId: string,
  store: CollectionStore = getDefaultStore()
): Promise<FigmaRecord[]> {
  const records = await listFigmaRecords(store);
