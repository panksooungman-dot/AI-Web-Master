...
describe("proxy() — route protection (proxy.ts, release hardening v1.0)", () => {
  ...
  describe("/developer/** page requests", () => {
    it("redirects to /login with a redirect target when there is no session", () => {
      ...
      const response = proxy(new NextRequest("http://localhost/developer/terminal"));
  ...
  describe("/admin/** page requests (no pages exist yet, but the rule is ready)", () => {
    it("returns 403 for a logged-in 'developer' role", () => {
      resolveSessionUserMock.mockReturnValue(userWithRole("developer"));
  ...
  describe("gated /api/** requests", () => {
    it("returns 401 JSON (not a redirect) when there is no session", async () => {
      resolveSessionUserMock.mockReturnValue(null);
      const response = proxy(new NextRequest("http://localhost/api/design/requirements"));
    ...
    it("returns 403 JSON for a role without developer access", async () => {
      resolveSessionUserMock.mockReturnValue(userWithRole("user"));
      const response = proxy(new NextRequest("http://localhost/api/websites"));
    ...
    it("allows a developer role through to a gated API route", () => {
      resolveSessionUserMock.mockReturnValue(userWithRole("developer"));
      const response = proxy(new NextRequest("http://localhost/api/metrics"));
      expect(response.status).toBe(200);
    });
  });

  describe("ungated /api/** exceptions", () => {
    it("lets /api/auth/login through with no session (must stay reachable to log in)", () => {
      resolveSessionUserMock.mockReturnValue(null);
      const response = proxy(new NextRequest("http://localhost/api/auth/login"));
      expect(response.status).toBe(200);
    });

    it("lets /api/terminal through even for a 'user' role (documented CLI-compat exception)", () => {
      resolveSessionUserMock.mockReturnValue(userWithRole("user"));
      const response = proxy(new NextRequest("http://localhost/api/terminal"));
      expect(response.status).toBe(200);
    });

    it("lets /api/devserver/status through with no session", () => {
      resolveSessionUserMock.mockReturnValue(null);
      const response = proxy(new NextRequest("http://localhost/api/devserver/status"));
      expect(response.status).toBe(200);
    });
  });
  ...
  describe("/projects (login required, no specific role)", () => {
    it("redirects to /login when there is no session", () => {
      resolveSessionUserMock.mockReturnValue(null);
      const response = proxy(new NextRequest("http://localhost/projects"));
      expect(response.status).toBe(307);
    });

    it("allows a plain 'user' role through (out of RBAC scope)", () => {
      resolveSessionUserMock.mockReturnValue(userWithRole("user"));
      const response = proxy(new NextRequest("http://localhost/projects"));
      expect(response.status).toBe(200);
    });
  ...
  describe("/login redirect-when-already-authenticated", () => {
    it("sends a developer to /developer", () => {
      resolveSessionUserMock.mockReturnValue(userWithRole("developer"));
    ...
    it("sends a plain user to the public home page", () => {
      resolveSessionUserMock.mockReturnValue(userWithRole("user"));
      const response = proxy(new NextRequest("http://localhost/login"));
      expect(new URL(response.headers.get("location")!).pathname).toBe("/");
    });

    it("does not redirect an anonymous visitor away from /login", () => {
      resolveSessionUserMock.mockReturnValue(null);
      const response = proxy(new NextRequest("http://localhost/login"));
      expect(response.status).toBe(200);
    });
  ...
  describe("public marketing pages", () => {
    it("are never gated, logged in or not", () => {
      resolveSessionUserMock.mockReturnValue(null);
      expect(proxy(new NextRequest("http://localhost/")).status).toBe(200);
      expect(proxy(new NextRequest("http://localhost/about")).status).toBe(200);
    });
  });
});
