...
- **`Security Analysis` CI가 `main`에서 상시 실패하던 오탐**(`.github/workflows/security.yml`) —
  ...
  탐지력 손실이 없다.
  - 개인키 헤더는 오히려 **탐지 범위가 넓어졌다** — 기존 패턴은 `-----BEGIN PRIVATE KEY-----`
    literal 하나뿐이라 `-----BEGIN RSA PRIVATE KEY-----`·`-----BEGIN OPENSSH PRIVATE KEY-----`를
    **놓치고 있었다**(실측 확인: 기존 0건 탐지 → 신규 2건 탐지). `-----BEGIN [A-Z ]*PRIVATE KEY-----`로 교체.
