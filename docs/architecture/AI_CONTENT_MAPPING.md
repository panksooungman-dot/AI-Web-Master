...
- [ ] 남은 7개 직군(`business-analyst`·`devops-engineer`·`frontend-engineer`·`product-manager`·`qa-engineer`·`solution-architect`·`technical-writer`) + `prompts/*.md` 6개로 확대
- [ ] 전체 완료 후 `agents/README.md`·`prompts/README.md`에 안내 문구 추가(`agents/README.md`는 이번까지 2건 "Status" 열로 반영 완료, `prompts/README.md`는 아직 미반영)
...
| 나머지 7개(`business-analyst`·`devops-engineer`·`frontend-engineer`·`product-manager`·`qa-engineer`·`solution-architect`·`technical-writer`) | 미반영 | 미반영(대상: 각 `agents/<role>.md`) | 없음(대응 소스 있음, CS-08 병합 대기) |
...
**진행 방식 확정(2026-07-19)**: 사용자가 (b)안을 선택 — `status`/`source` frontmatter는 일괄 반영하지 않고, CS-08 개별 Pilot이 그 파일을 병합할 때마다 함께 반영한다. `backend-engineer`가 이 방식으로 반영된 두 번째 사례다.
