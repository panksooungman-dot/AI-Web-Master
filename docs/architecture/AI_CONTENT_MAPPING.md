...
| `prompts/planner.md` | `business-analyst/`, `product-manager/` | Expected Output Structure | ✅ Merged (Phase 2, 2026-07-19) |
| `prompts/coder.md` | `backend-engineer/`, `frontend-engineer/`, `ai-engineer/` | Expected Output Structure (Coding) | ✅ Merged (Phase 2 Pilot, 2026-07-19) |
| `prompts/reviewer.md` | `solution-architect/`, `devops-engineer/`, `qa-engineer/` | Expected Output Structure (Review) | ✅ Merged (Phase 2, 2026-07-19) |
| `prompts/tester.md` | `qa-engineer/` (주 대상) 외 4개 | Expected Output Structure (Testing) | ✅ Merged (Phase 2, 2026-07-19) |
| `prompts/documenter.md` | `technical-writer/`, `product-manager/`, `qa-engineer/` | Expected Output Structure (Documentation) | ✅ Merged (Phase 2, 2026-07-19) |
| `prompts/system.md` | 15개 전체 공통 | 병합 대신 각 SKILL.md 상단에 "전역 규칙은 prompts/system.md 참고" 각주만 추가(내용 복제 금지 — 이미 전역이므로) | ✅ Footnote 적용 완료(15/15, 2026-07-19). `system.md` 자신은 축소하지 않음(계획대로) |

**실행 순서 원칙**(CS-08과 동일): 1개 직군(`ai-engineer`)을 시범 사례로 먼저 진행 후 나머지 8개 + `prompts/*.md` 6개로 확대. **진행 상황(2026-07-19)**: `agents/*.md` **9개 전부 완료(9/9)** — Pilot(`ai-engineer`·`backend-engineer`·`frontend-engineer`) → Batch 1(`devops-engineer`·`qa-engineer`·`technical-writer`) → Batch 2(`business-analyst`·`product-manager`·`solution-architect`, `# Collaboration` 헤더가 없어 `# Workflow` 직전에 삽입, `product-manager`는 기존 `# Decision Framework`와의 관계를 각주로 구분). **`prompts/*.md` 6개 전부 완료(6/6)** — Phase 2 Pilot(`coder.md`) → `planner.md`(business-analyst·product-manager) → `reviewer.md`(solution-architect·devops-engineer·qa-engineer, fan-in≥2 대상에 소스명 헤딩 `(Review)` 규칙 첫 적용) → `documenter.md`(technical-writer·product-manager·qa-engineer) → `tester.md`(qa-engineer·backend-engineer·frontend-engineer·ai-engineer·devops-engineer, fan-out 5 최대) → `system.md`(15개 전체 각주만, 병합 아님). CS-08 전체(Phase 1 + Phase 2) **완료(15/15)**.
...
- [x] **`prompts/*.md` 6/6 전부 완료(Phase 2).** `docs/architecture/P3_PHASE2_REVIEW.md`가 재계산한 순서(`coder → planner → reviewer → documenter → tester → system`)대로 진행, fan-in ≥2 대상에는 소스명 헤딩(`(Coding)`/`(Planning)`/`(Review)`/`(Documentation)`/`(Testing)`)을 적용해 `qa-engineer`(fan-in 4, 최대)·`product-manager`(fan-in 2)에서 헤딩 충돌 없이 병존 확인. `prompts/README.md`에 "상세는 `skills/experts/` 참고" 안내 문구 반영 — 2026-07-19 완료
- [x] 각 단계 개별 커밋(CS-08 롤백 방침과 동일) — Refactoring Phase 2 세션에서 실행
...
### 9.3 준수 현황 (2026-07-19 기준, Phase 2 전체 완료 후 갱신)

| 직군 | status | source / sources | fan-in (prompt 출처 수) | Decision Authority / Handoff |
|---|---|---|---:|---|
| `ai-engineer` | `merged` | `sources` 배열(agent: `agents/ai-engineer.md`, prompt: `prompts/coder.md`, `prompts/tester.md`) | 2 | ✅ 있음 |
| `backend-engineer` | `merged` | `sources` 배열(agent: `agents/backend-engineer.md`, prompt: `prompts/coder.md`, `prompts/tester.md`) | 2 | ✅ 있음 |
| `frontend-engineer` | `merged` | `sources` 배열(agent: `agents/frontend-engineer.md`, prompt: `prompts/coder.md`, `prompts/tester.md`) | 2 | ✅ 있음 |
| `devops-engineer` | `merged` | `sources` 배열(agent: `agents/devops-engineer.md`, prompt: `prompts/reviewer.md`, `prompts/tester.md`) | 2 | ✅ 있음 |
| `qa-engineer` | `merged` | `sources` 배열(agent: `agents/qa-engineer.md`, prompt: `prompts/reviewer.md`, `prompts/documenter.md`, `prompts/tester.md`) | **4(최대)** | ✅ 있음 |
| `technical-writer` | `merged` | `sources` 배열(agent: `agents/technical-writer.md`, prompt: `prompts/documenter.md`) | 1 | ✅ 있음 |
| `business-analyst` | `merged` | `sources` 배열(agent: `agents/business-analyst.md`, prompt: `prompts/planner.md`) | 1 | ✅ 있음(`# Workflow` 직전 삽입) |
| `product-manager` | `merged` | `sources` 배열(agent: `agents/product-manager.md`, prompt: `prompts/planner.md`, `prompts/documenter.md`) | 2 | ✅ 있음(`# Workflow` 직전 삽입, `# Decision Framework`와의 관계 각주 포함) |
| `solution-architect` | `merged` | `sources` 배열(agent: `agents/solution-architect.md`, prompt: `prompts/reviewer.md`) | 1 | ✅ 있음(`# Workflow` 직전 삽입) |
| `data-engineer`·`fullstack-engineer`·`scrum-master`·`security-engineer`·`ui-designer`·`ux-designer` | 미반영(status/sources 없음) | 대응 agent/prompt 소스 자체가 없음 | 0 | 없음(대응 소스 없음, 신규 작성은 이번 CS-08 범위 밖) |

모든 병합 대상 9개 파일이 단일 `source:` 문자열이 아닌 `sources:` 배열로 전환됨(9.1.1 구조). `system.md` 각주는 `sources`/`status`와 무관한 별도 처리(9.4 참고) — 15개 전부에 적용되지만 frontmatter를 건드리지 않는다.

**진행 방식 확정(2026-07-19)**: 사용자가 (b)안을 선택 — `status`/`source` frontmatter는 일괄 반영하지 않고, CS-08 개별 Pilot/Batch/Phase가 그 파일을 병합할 때마다 함께 반영한다. Pilot(3) + Batch 1(3) + Batch 2(3) = Phase 1의 9/15가 이 방식으로 반영되었고, **Phase 2(`planner`→`coder`→`reviewer`→`documenter`→`tester`)가 이어서 9개 전부를 다중 출처 `sources:` 배열로 전환 완료했다**(9.1.1 참고) — `qa-engineer`가 4개 출처로 가장 붐비는 대상이었으나 헤딩 충돌 없이 병존시켰다.

### 9.4 이번에 실행한 것

`skills/experts/ai-engineer/SKILL.md`의 frontmatter에 `status: merged`, `source: agents/ai-engineer.md (merged 2026-07-19)`를 추가해 이 표준의 기준 사례(reference implementation)로 삼았다. 이후 `backend-engineer`·`frontend-engineer`(Pilot #2·#3), `devops-engineer`·`qa-engineer`·`technical-writer`(Batch 1), `business-analyst`·`product-manager`·`solution-architect`(Batch 2)에도 동일하게 적용해 총 9개 파일에 반영 완료 — `agents/*.md` 9개 전부. 2026-07-19, Phase 2 Pilot(`prompts/coder.md`) 실행 중 단일 `source:` 문자열로는 다중 출처를 표현할 수 없다는 설계 공백을 발견해, `ai-engineer`·`backend-engineer`·`frontend-engineer` 3개(당시 2개 이상 출처를 가진 파일 전부)의 frontmatter를 `sources:` 배열 구조로 전환했다(9.1.1 참고).

**Phase 2 전체 완료 (2026-07-19, 이어지는 Refactoring Phase 2 세션)**: `planner.md`(business-analyst·product-manager) → `reviewer.md`(solution-architect·devops-engineer·qa-engineer) → `documenter.md`(technical-writer·product-manager·qa-engineer) → `tester.md`(qa-engineer·backend-engineer·frontend-engineer·ai-engineer·devops-engineer) 순으로 병합, 나머지 6개 파일(`devops-engineer`·`qa-engineer`·`technical-writer`·`business-analyst`·`product-manager`·`solution-architect`)도 두 번째 이상 출처가 생기며 전부 `sources:` 배열로 전환됨 — 이제 9개 병합 대상 전부가 배열 구조로 통일됐다. `qa-engineer`는 4개 출처(agent 1 + prompt 3)를 받는 최대 fan-in 대상이라, `P3_PHASE2_REVIEW.md`가 사전에 확정한 "fan-in ≥2 대상은 소스명을 헤딩에 포함"(`# Expected Output Structure (Review)`/`(Documentation)`/`(Testing)`) 규칙을 예외 없이 적용해 헤딩 충돌 없이 4개 섹션을 병존시켰다. 마지막으로 `system.md`는 병합하지 않고, 15개 `SKILL.md` 전체(frontmatter 변경 없이 H1 직후 위치)에 "전역 규칙은 `prompts/system.md` 참고" 각주 한 줄만 추가했다 — `data-engineer`·`fullstack-engineer`·`scrum-master`·`security-engineer`·`ui-designer`·`ux-designer` 6개(대응 agent/prompt 소스가 없어 `status`/`sources`는 여전히 미반영)도 이 각주는 동일하게 받는다. 이것으로 CS-08(Phase 1 + Phase 2) 전체가 완료됐다.

---

*본 문서는 계획서였습니다. `agents/*.md` 9개(Pilot 3 + Batch 1 3 + Batch 2 3) + `prompts/*.md` 6개(Phase 2 Pilot `coder` + `planner`/`reviewer`/`documenter`/`tester`/`system`), 총 **CS-08 전체 15/15 실행 완료**(2026-07-19). 남은 리팩터링 백로그는 `CHANGESET.md`의 CS-09~CS-14(component-marker 공유화·레거시 안내·marketplace 카테고리·skills 존치 결정·상가분양센터·worktree 정리)를 참고하세요 — 전부 이 문서(agents/prompts→skills 병합)와는 별개 항목입니다.*
