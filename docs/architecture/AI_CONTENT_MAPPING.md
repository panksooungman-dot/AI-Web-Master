...
**실행 순서 원칙**(CS-08과 동일): 1개 직군(`ai-engineer`)을 시범 사례로 먼저 진행 후 나머지 8개 + `prompts/*.md` 6개로 확대. **진행 상황(2026-07-19)**: `agents/*.md` 9개 중 6개 완료(`ai-engineer`·`backend-engineer`·`frontend-engineer`·`devops-engineer`·`qa-engineer`·`technical-writer`), 남은 3개(`business-analyst`·`product-manager`·`solution-architect`)는 `CHECKPOINT_REVIEW.md`에서 확인된 대로 대상 SKILL.md에 `# Collaboration` 헤더가 없어 삽입 지점 조정이 필요함(Batch 2 대상).
...
- [ ] 남은 3개 직군(`business-analyst`·`product-manager`·`solution-architect`, Batch 2) + `prompts/*.md` 6개로 확대. Batch 2는 대상 SKILL.md에 `# Collaboration` 헤더가 없어 삽입 지점을 `# Workflow` 직전으로 조정 필요(`CHECKPOINT_REVIEW.md` 5번 참고), `product-manager`는 기존 `# Decision Framework`와의 관계 각주 추가 필요
- [ ] 전체 완료 후 `agents/README.md`·`prompts/README.md`에 안내 문구 추가(`agents/README.md`는 이번까지 6건 "Status" 열로 반영 완료 + "Standard Agent Template" 섹션에 Merged/Pending 구분 각주 추가 완료, `prompts/README.md`는 아직 미반영)
...
| Decision Authority | 없음(`agents/*.md`에만 존재) | ⚠️ `ai-engineer`·`backend-engineer`·`frontend-engineer`·`devops-engineer`·`qa-engineer`·`technical-writer` 6/15 충족(CS-08 Pilot+Batch 1로 병합됨), 나머지 3/15(`business-analyst`·`product-manager`·`solution-architect`)는 대응 소스 있음(Batch 2 대기, `# Collaboration` 헤더 없어 삽입 지점 조정 필요), 대응 소스가 없는 6개(`data-engineer`·`fullstack-engineer`·`scrum-master`·`security-engineer`·`ui-designer`·`ux-designer`)는 신규 작성 필요 |
...
| 나머지 3개(`business-analyst`·`product-manager`·`solution-architect`, Batch 2) | 미반영 | 미반영(대상: 각 `agents/<role>.md`) | 없음(대응 소스 있음, `# Collaboration` 헤더 없어 삽입 지점 조정 필요) |
...
**진행 방식 확정(2026-07-19)**: 사용자가 (b)안을 선택 — `status`/`source` frontmatter는 일괄 반영하지 않고, CS-08 개별 Pilot/Batch가 그 파일을 병합할 때마다 함께 반영한다. Batch 1(`devops-engineer`·`qa-engineer`·`technical-writer`)까지 총 6/15가 이 방식으로 반영 완료.

### 9.4 이번에 실행한 것

`skills/experts/ai-engineer/SKILL.md`의 frontmatter에 `status: merged`, `source: agents/ai-engineer.md (merged 2026-07-19)`를 추가해 이 표준의 기준 사례(reference implementation)로 삼았다. 이후 `backend-engineer`·`frontend-engineer`(Pilot #2·#3)와 `devops-engineer`·`qa-engineer`·`technical-writer`(Batch 1)에도 동일하게 적용해 총 6개 파일에 반영 완료. 기존 7개 키·본문 섹션은 전혀 건드리지 않았다.

---

*본 문서는 계획서입니다. `agents/*.md` 9개 중 6건(`ai-engineer`·`backend-engineer`·`frontend-engineer`·`devops-engineer`·`qa-engineer`·`technical-writer`, 2026-07-19) 실행 완료 — 나머지 항목(Batch 2: `business-analyst`·`product-manager`·`solution-architect`, `prompts/*.md` 6개)은 사용자 승인 후 별도 세션에서 진행합니다.*
