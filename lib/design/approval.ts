...
function applyApprovalAction(
  ...
  const review = getReview(id, baseDir);
  if (!review) {
  ...
  const updated = transitionReviewStatus(id, transition.to, options, baseDir);
  return updated
    ? { success: true, record: updated }
    : { success: false, errorCode: "not_found", error: `Review "${id}"을(를) 찾을 수 없습니다.` };
}

export function approveReview(id: string, options: ApprovalOptions = {}, baseDir?: string): ApprovalResult {
  return applyApprovalAction(id, "approve", options, baseDir);
}

export function rejectReview(id: string, options: ApprovalOptions = {}, baseDir?: string): ApprovalResult {
  return applyApprovalAction(id, "reject", options, baseDir);
}

export function requestRevision(id: string, options: ApprovalOptions = {}, baseDir?: string): ApprovalResult {
  return applyApprovalAction(id, "revision", options, baseDir);
}

export function cancelApproval(id: string, options: ApprovalOptions = {}, baseDir?: string): ApprovalResult {
  return applyApprovalAction(id, "cancel", options, baseDir);
}
...
export function applyApproval(
  ...
  return applyApprovalAction(id, action, options, baseDir);
}
