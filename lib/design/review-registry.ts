...
function registryPath(baseDir: string): string {
  return path.join(baseDir, "design-reviews.json");
}
...
function readRegistry(baseDir: string): ReviewRecord[] {
...
function writeRegistry(baseDir: string, records: ReviewRecord[]): void {
  ensureRegistryFile(baseDir);
 ...
 */
export function createReview(entry: CreateReviewEntry, baseDir: string = DEFAULT_BASE_DIR): ReviewRecord {
...
/** 최신순(newest first). */
export function listReviews(baseDir: string = DEFAULT_BASE_DIR): ReviewRecord[] {
  return [...readRegistry(baseDir)].reverse();
}

export function getReview(id: string, baseDir: string = DEFAULT_BASE_DIR): ReviewRecord | null {
  return readRegistry(baseDir).find((record) => record.id === id) ?? null;
}

/** 특정 Claude Design 산출물에서 시작된 Review만(최신순 — 즉 최신 version이 먼저). */
export function listReviewsForClaudeDesign(
  claudeDesignId: string,
  baseDir: string = DEFAULT_BASE_DIR
): ReviewRecord[] {
  return listReviews(baseDir).filter((record) => record.claudeDesignId === claudeDesignId);
}
...
export function archiveReview(
  ...
  return transitionReviewStatus(id, "archived", { actor: options.actor, note: options.note ?? "보관 처리" }, baseDir);
}
