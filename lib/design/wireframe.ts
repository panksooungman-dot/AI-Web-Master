 ...
 * 생성한다("Wireframe Generator on top of Phase 2"). 이 파일은 타입 + fs-JSON registry,
...
function registryPath(baseDir: string): string {
  return path.join(baseDir, "design-wireframes.json");
}
...
function readRegistry(baseDir: string): WireframeRecord[] {
  ensureRegistryFile(baseDir);

  try {
    const raw = fs.readFileSync(registryPath(baseDir), "utf-8");
    const parsed: unknown = JSON.parse(raw);
    return Array.isArray(parsed) ? (parsed as WireframeRecord[]) : [];
  } catch {
    return [];
  }
}

function writeRegistry(baseDir: string, records: WireframeRecord[]): void {
  ensureRegistryFile(baseDir);
...
/** 최신순(newest first). */
export function listWireframes(baseDir: string = DEFAULT_BASE_DIR): WireframeRecord[] {
  return [...readRegistry(baseDir)].reverse();
}

export function getWireframe(id: string, baseDir: string = DEFAULT_BASE_DIR): WireframeRecord | null {
  return readRegistry(baseDir).find((record) => record.id === id) ?? null;
}

/** 특정 Storyboard에서 생성된 Wireframe만(최신순). */
export function listWireframesForStoryboard(
  storyboardId: string,
  baseDir: string = DEFAULT_BASE_DIR
): WireframeRecord[] {
  return listWireframes(baseDir).filter((record) => record.storyboardId === storyboardId);
}
