...
# 관리자 권한이 필요 없고, `npm uninstall -g @ai-business-os/cli`로
# 언제든 완전히 제거할 수 있다.
...
try {
    ...
    if ($installOk) {
        ...
        if (-not $aiShimExists) {
            ...
            & npm uninstall -g "@ai-business-os/cli" 2>&1 | ForEach-Object { Write-Host "  $_" -ForegroundColor DarkGray }
...
$aiFunctionScriptPath = Join-Path $npmPrefix "node_modules\@ai-business-os\cli\shell\ai-function.ps1"
